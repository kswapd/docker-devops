			...
			if (DATA.errorMsg && flag) {
				...
				if (DATA.data) {
					...
					if ($("#OLD_JSON_STRING").val() != body) {
						...
						batchMsg += "批处理日期：";
						if (DATA.data.batchStatus.RUN_DATE && DATA.data.batchStatus.RUN_DATE != "")
							batchMsg += DATA.data.batchStatus.RUN_DATE;
						batchMsg += " 批处理开始时间：";
						if (DATA.data.batchStatus.START_TIME && DATA.data.batchStatus.START_TIME != "")
							batchMsg += DATA.data.batchStatus.START_TIME;
						batchMsg += " 批处理完成时间：";
						if (DATA.data.batchStatus.END_TIME && DATA.data.batchStatus.END_TIME != "")
							batchMsg += DATA.data.batchStatus.END_TIME;
